...
export default function Signin() {
  ...
  return (
    <div className='w-full h-screen bg-cover bg-center bg-no-repeat flex justify-center items-center' style={{ backgroundImage: `url(${bg})` }}>
      ...
      shadow-lg shadow-gray-700 flex flex-col items-center justify-center gap-[20px] p-2' onSubmit={handlesignin}>
        ...
        <input type="text" required placeholder='Enter your name or email' className='
        ...
        <div className='w-full h-9 border-2 border-white bg-transparent text-white rounded-full '>
          ...
          {!showpassword && <IoEye className='absolute bottom-53 right-5  text-white cursor-pointer' onClick={() => setShowpassword(true)} />}
          {showpassword && <IoEyeOff className='absolute bottom-53 right-5  text-white cursor-pointer' onClick={() => setShowpassword(false)} />}
        </div>
        {err.length>0 && <p className='text-red-500 text-lg'>
          *{err}
          </p>}
        <button className='min-w-10 mt-10 h-9 bg-white rounded-2xl text-black font-semibold text-2xl active:scale-75 shadow-md shadow-amber-300' disabled={loading}>{loading? "loading..." :"Sign In"}</button>
        <p className='text-white'>Don`t` have an account<span className='text-blue-500 cursor-pointer ' onClick={() => navigate('/signup')}>Sign Up</span></p>
      </form>
    </div>
  )
}
